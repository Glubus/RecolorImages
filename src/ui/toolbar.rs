...
pub fn toolbar(ui: &mut egui::Ui, app: &mut ImageApp, ctx: &egui::Context) {
    ui.horizontal(|ui| {
        if ui.button("Charger Images").clicked() && let Some(paths) = FileDialog::new().pick_files() {
            for path in paths {
        ...
        if ui.button("Sauvegarder toutes les images").clicked() && let Some(folder) = FileDialog::new().pick_folder() {
